...
exports.getAnswerResearchAssistant = void 0;
...
async function getSources(message) {
    ...
    try {
        const response = await fetch(`https://api.search.brave.com/res/v1/web/search?q=${encodedMessage}&count=10&search_lang=vi`, {
            headers: {
        ...
        const final = jsonResponse.web.results.map((result) => ({
            title: result.title,
            link: result.url,
...
async function processAndVectorizeContent(contents, query, textChunkSize = 1000, textChunkOverlap = 400, numberOfSimilarityResults = 4) {
    try {
        const embeddings = new openai_1.OpenAIEmbeddings();
        for (let i = 0; i < contents.length; i++) {
            ...
            if (content.html.length > 0) {
                try {
                    const splitText = await new text_splitter_1.RecursiveCharacterTextSplitter({
                        chunkSize: textChunkSize,
                        chunkOverlap: textChunkOverlap,
                    }).splitText(content.html);
                    const vectorStore = await memory_1.MemoryVectorStore.fromTexts(splitText, { title: content.title, link: content.link }, embeddings);
...
async function getAnswerResearchAssistant(datas) {
    // Phân tích datas
    ...
    const b = Date.now();
    console.log("===============vectorResults took " + (b - a) + " ms.===================");
...
exports.getAnswerResearchAssistant = getAnswerResearchAssistant;
//# sourceMappingURL=research_assistant.js.map