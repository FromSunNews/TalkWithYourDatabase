...
const followUpQuestions = async (sources: SearchResult[]): Promise<any> => {
  const groqResponse: any = await openai.chat.completions.create({
    messages: [
      {
        role: "system",
        content: `
...
export async function getAnswerResearchAssistant(datas: ConfigOptions): Promise<any> {
  // Phân tích datas
